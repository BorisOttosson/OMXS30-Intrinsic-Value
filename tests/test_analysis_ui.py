...
class CashFlowAnalysisUiTests(unittest.TestCase):
    ...
    def test_eqt_uses_a_dedicated_traceable_sum_of_the_parts_page(self):
        ...
        self.assertEqual(config["feeBusiness"]["growthAdjustment"], {"base": 0, "bull": 2, "bear": -2})
        ...
        self.assertIn('id="eqtForecastEditor"', self.html)
        self.assertIn('data-eqt-field="eqtFeeEbitdaGrowth"', self.html)
        self.assertIn('data-eqt-field="eqtTerminalMultiple"', self.html)
        self.assertIn("How the EQT sum-of-the-parts works", self.html)
        self.assertIn("const forecastRows = Array.from", self.javascript)
        self.assertIn("terminalFeeBusinessValueEurm / discountFactor", self.javascript)
        self.assertIn("No interim EBITDA is added as cash flow", self.javascript)
        self.assertIn("company[eqtField] = numberOrNull(event.target.value)", self.javascript)
        eqt_function = self.javascript.split("function calculateEqtSotp", 1)[1].split("function calculateBolidenCommodityCycle", 1)[0]
        self.assertNotIn("getSelectedGrowthAssumption", eqt_function)
        self.assertNotIn("consensusGrowth", eqt_function)
        self.assertNotIn("growth5y", eqt_function)
